...
class InverseRadonTransform:
    def transform(self, sinogram: [[float]], ray_calculator: RayCalculator):
        image_size = (ray_calculator.image_width, ray_calculator.image_height)
        ...
        for sinogram_vector_number in range(0, rays_quantity):
            self.__transform_vector(image, lines_quantity_per_pixel, detectors_quantity, sinogram, ray_calculator,
    ...
    def __transform_vector(self, image: [[float]], lines_quantity_per_pixel: [[int]], detectors_quantity: int,
                           sinogram: [[float]], tomograph: RayCalculator, vector_number: int):
    ...
    def get_partial_result(self, sinogram: [[float]], tomograph: RayCalculator, progress: int, image: [[float]] = None,
